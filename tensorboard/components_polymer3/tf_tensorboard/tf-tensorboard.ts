...
class TfTensorboard extends PolymerElement {
    static readonly template = html `<paper-dialog with-backdrop="" id="settings">
      <h2>Settings</h2>
    ...
    _activeDashboardsLoadState: string = tf_tensorboard.ActiveDashboardsLoadState.NOT_LOADED;
    @property({
        type: String,
    ...
    get _homePath(): string {
        ...
        if (!isHttp) {
            throw new RangeError(`Expect 'homePath' to be of http: or https:. ${homePath}`);
        }
        if (!sameOrigin) {
            throw new RangeError(`Expect 'homePath' be a path or have the same origin. ${homePath} vs. ${location.origin}`);
        }
    ...
    _isDashboardActive(disabledDashboards, activeDashboards, dashboardDatum) {
        if ((disabledDashboards || '')
            .split(',')
            .indexOf(dashboardDatum.plugin) >= 0) {
            // Explicitly disabled.
            return false;
        }
     ...
     */
    _ensureSelectedDashboardStamped(dashboardRegistry, containersStamped, activeDashboards, selectedDashboard) {
        if (!activeDashboards ||
            !selectedDashboard ||
        ...
        const container = this.$$(`.dashboard-container[data-dashboard=${selectedDashboard}]`);
        if (!container) {
        ...
        if (container.children.length === 0) {
            ...
            switch (loadingMechanism.type) {
                ...
                case 'IFRAME': {
                    this._renderPluginIframe(container, selectedDashboard, loadingMechanism);
                    break;
                }
                default: {
    ...
    _selectedDashboardComponent() {
        ...
        var dashboard = this.$$(`.dashboard-container[data-dashboard=${selectedDashboard}] #dashboard`);
        return dashboard;
    }
    ready() {
    ...
    _onTemplateChanged() {
        ...
        for (const container of this.root.querySelectorAll('.dashboard-container')) {
    ...
    get _dashboardRegistry(): object {
        ...
        const registry = {};
        for (const [name, legacyMetadata] of Object.entries(tf_tensorboard.dashboardRegistry)) {
            registry[name] = {
                ...
                switch (backendMetadata.loading_mechanism.type) {
                    case 'NONE':
                        ...
                        if (registry[name] == null) {
                            console.warn('Plugin has no loading mechanism and no baked-in registry entry: %s', name);
                        }
                        continue;
                    case 'CUSTOM_ELEMENT':
                        loadingMechanism = {
                    ...
                    case 'IFRAME':
                        ...
                        break;
                    default:
                        console.warn('Unknown loading mechanism for plugin %s: %s', name, backendMetadata.loading_mechanism);
                        continue;
                }
                if (loadingMechanism == null) {
                    console.error('Invariant violation: loadingMechanism is %s for %s', loadingMechanism, name);
                }
                registry[name] = {
                    plugin: name,
    ...
    _fetchPluginsListing() {
        ...
        const onFailure = () => {
            if (this._activeDashboardsLoadState ===
                ...
                console.warn('Failed to reload the set of active plugins; using old value.');
            }
        };
    ...
    @computed("_activeDashboardsLoaded", "_activeDashboards", "_selectedDashboard")
    ...
    @computed("_activeDashboardsLoaded", "_dashboardRegistry", "_selectedDashboard")
